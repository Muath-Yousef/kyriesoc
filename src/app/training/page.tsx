      ...
      <div className="min-h-screen flex items-center justify-center py-20 px-6">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          <div className="text-center mb-10">
            <div className="w-14 h-14 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center mx-auto mb-5 text-2xl">🛡️</div>
            <h1 className="text-3xl font-extrabold tracking-tight">Security Awareness Training</h1>
            <p className="text-neutral-500 mt-2 text-sm">Sign in to access your training modules. Need an account? <a href="/portal/login" className="text-emerald-400 hover:underline">Register here →</a></p>
          </div>
  ...
  return (
    <div className="min-h-screen py-16">
      <div className="container mx-auto px-6 max-w-4xl">
        ...
        {/* Header */}
        <div className="text-center mb-10">
          <p className="font-mono text-xs text-emerald-400 uppercase tracking-[0.3em] mb-3">SOC Root Academy</p>
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-2">
            Cybersecurity Awareness <span className="text-emerald-400">Training</span>
          </h1>
          ...
          <div className="mb-12">
            <div className="flex items-center justify-between gap-1 overflow-x-auto pb-2">
              {PHASES.map((p, idx) => {
                ...
                return (
                  <div key={p.id} className="flex items-center gap-1 flex-shrink-0">
                    ...
                    {idx < PHASES.length - 1 && <div className={`w-2 lg:w-6 h-px flex-shrink-0 ${isDone ? "bg-emerald-500/40" : "bg-white/5"}`} />}
                  </div>
                );
              })}
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <div className={`px-3 py-1.5 rounded-full text-[10px] font-mono ${
                  currentPhase === "cert" ? "bg-emerald-500 text-black font-bold" : "bg-white/5 text-neutral-600 border border-white/5"
                }`}>
                  🎖
                </div>
              </div>
            ...
            {typeof currentPhase === "number" && (
              <div className="mt-4 bg-white/5 rounded-full h-2 overflow-hidden">
                <div className="bg-gradient-to-r from-emerald-600 to-emerald-400 h-full rounded-full transition-all duration-500"
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
            <div className="border border-white/8 bg-white/[0.02] p-8 rounded-2xl">
              <h2 className="text-2xl font-bold mb-4 flex items-center gap-3">
                <BookOpen className="w-6 h-6 text-emerald-500" /> Welcome to the Program
              </h2>
              ...
              <div className="flex items-center gap-3 p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/20 mb-6">
              ...
              <button onClick={handleStart}
                className="bg-emerald-500 hover:bg-emerald-400 text-black font-bold px-8 py-4 rounded-xl transition-all hover:shadow-[0_0_25px_rgba(16,185,129,0.35)] flex items-center gap-2 text-sm uppercase tracking-wider">
            ...
            <motion.div key={currentPhase} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              ...
              <div className="border border-emerald-500/20 bg-emerald-500/5 p-6 rounded-2xl">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <p className="font-mono text-[10px] text-emerald-400 uppercase tracking-widest">Phase {phase.id} of {PHASE_COUNT} · {phase.duration}</p>
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowContent(true)}
                  className={`px-4 py-2 rounded-lg text-xs font-mono transition-all ${showContent ? "bg-emerald-500 text-black font-bold" : "bg-white/5 text-neutral-500 hover:bg-white/10"}`}>
                  📖 Study Material
                </button>
                ...
                {showContent ? (
                  <motion.div key="content" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-5">
                    {phase.sections.map((sec, si) => (
                      <div key={si} className="border border-white/8 bg-white/[0.02] p-6 rounded-2xl">
                        <h3 className="font-bold text-white text-lg mb-3 flex items-center gap-2">
                          <span className="w-6 h-6 rounded bg-emerald-500/10 border border-emerald-500/25 flex items-center justify-center text-emerald-400 text-[10px] font-mono">{si + 1}</span>
                          {sec.title}
                        </h3>
                    ...
                    <div className="text-center pt-4">
                      <button onClick={() => setShowContent(false)}
                        className="bg-emerald-500 hover:bg-emerald-400 text-black font-bold px-8 py-3.5 rounded-xl transition-all text-sm">
                  ...
                  <motion.div key="quiz" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <div className="border border-emerald-500/20 bg-emerald-500/5 p-6 rounded-2xl">
                      <h3 className="text-lg font-bold text-emerald-400 mb-2">Phase {phase.id} Assessment</h3>
                      ...
                      <button onClick={() => submitQuiz(currentPhase)}
                        className="w-full bg-emerald-500 hover:bg-emerald-400 text-black font-bold py-4 rounded-xl transition-all text-sm uppercase tracking-wider">
                        Submit Phase {phase.id} Assessment
                      </button>
                    </div>
                    <div className="text-center mt-4">
                      <button onClick={() => setShowContent(true)} className="text-xs text-neutral-600 hover:text-emerald-400 transition-colors">
        ...
        {currentPhase === "cert" && (
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className="space-y-8">
            <div className="border-2 border-dashed border-emerald-500/30 bg-emerald-500/5 rounded-2xl p-12 text-center relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-center gap-2 mb-2">
                  <span className="w-8 h-8 rounded bg-emerald-500 flex items-center justify-center">
                ...
                <p className="font-mono text-emerald-400 tracking-[0.3em] text-xs mb-1">SOC ROOT ACADEMY</p>
                ...
                <p className="text-4xl md:text-5xl font-extrabold text-emerald-400 mb-2 capitalize">{userName}</p>
