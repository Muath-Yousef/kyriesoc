    ...
    <div className="min-h-[calc(100vh-72px)] flex items-center justify-center py-20 px-6">
      ...
      >
        <div className="text-center mb-8">
          <div className="w-14 h-14 rounded-xl bg-teal-500/10 border border-teal-500/25 flex items-center justify-center mx-auto mb-4">
        ...
        {forgotMode ? (
          <div className="border border-white/8 bg-white/[0.02] rounded-none p-7 backdrop-blur-xl angular-cut bg-noise glass-dark">
            {forgotSent ? (
              <div className="text-center">
                ...
                <button onClick={() => { setForgotMode(false); setForgotSent(false); setError(""); }}
                  className="text-teal-500 hover:underline text-sm">← Back to Sign In
                </button>
              </div>
            ) : (
              <form onSubmit={handleForgotPassword} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="email" required placeholder="your@email.com" value={email}
                    ...
                    className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 outline-none focus:border-teal-500/60 transition-colors text-white placeholder-neutral-600 text-sm" />
                </div>
                {error && <p className="text-red-400 text-xs">{error}</p>}
                <button type="submit" disabled={loading}
                  className="w-full bg-teal-500 hover:bg-teal-400 disabled:opacity-50 text-black font-bold py-3.5 rounded-none transition-all text-sm angular-cut">
          ...
          <div className="border border-white/8 bg-white/[0.02] rounded-none p-7 backdrop-blur-xl angular-cut bg-noise glass-dark">
            ...
            <form onSubmit={handleVerify} className="space-y-4">
              <input type="text" required placeholder="SOC-XXXX"
                ...
                className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 outline-none focus:border-teal-500/60 transition-colors text-white font-mono tracking-widest text-center text-lg placeholder-neutral-600" />
            ...
            <p className="text-center text-xs text-neutral-600 mt-4">
              <button onClick={() => { setVerifyStep(false); setError(""); }} className="text-teal-500 hover:underline">← Back</button>
            </p>
          </div>
        ) : (
          <>
              ...
              {(["login", "register"] as const).map((t) => (
                <button key={t} onClick={() => { setTab(t); setError(""); }}
                  className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all capitalize ${
                    tab === t ? "bg-teal-500 text-black font-bold shadow" : "text-neutral-500 hover:text-neutral-300"
                  }`}>
            ...
            <div className="border border-white/8 bg-white/[0.02] rounded-none p-7 backdrop-blur-xl angular-cut bg-noise glass-dark">
              ...
              <div className="space-y-3 mb-6">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-none border border-white/10 bg-white/[0.03] hover:bg-white/[0.07] hover:border-white/20 transition-all text-sm font-medium text-neutral-300 angular-cut bg-noise glass-dark"
                >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="password" required placeholder="••••••••••••" value={password}
                    ...
                    className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 outline-none focus:border-teal-500/60 transition-colors text-white placeholder-neutral-600 font-mono text-sm" />
                </div>
                ...
                {tab === "login" && (
                  <div className="flex items-center justify-between text-xs mt-1">
                    ...
                    <button type="button" onClick={() => { setForgotMode(true); setError(""); }}
                      className="text-teal-500 hover:text-teal-400 transition-colors">Forgot password?
                    </button>
                  </div>
                )}

                <button type="submit" disabled={loading}
                  className="w-full mt-2 bg-teal-500 hover:bg-teal-400 disabled:opacity-50 text-black font-bold py-3.5 rounded-none transition-all hover:shadow-[0_0_20px_rgba(16,185,129,0.3)] text-sm angular-cut">
                  {loading ? (
